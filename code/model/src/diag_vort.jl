function vort(n)
  ...
  for j in 1:NJ
    for i in 1:NI
      for k in 1:NK
        vdx = 0.5 * ((v[i+1, j, k, 0] - v[i-1, j, k, 0]) * ux[i, j] + (v[i, j+1, k, 0] - v[i, j-1, k, 0]) * vx[i, j] + (v[i, j, k+1, 0] - v[i, j, k-1, 0]) * wx[i, j, k])
        udy = 0.5 * ((u[i+1, j, k, 0] - u[i-1, j, k, 0]) * uy[i, j] + (u[i, j+1, k, 0] - u[i, j-1, k, 0]) * vy[i, j] + (u[i, j, k+1, 0] - u[i, j, k-1, 0]) * wy[i, j, k])
        vor[i, j, k] = (vdx - udy) * UL * LENinv

        udx = 0.5 * ((u[i+1, j, k, 0] - u[i-1, j, k, 0]) * ux[i, j] + (u[i, j+1, k, 0] - u[i, j-1, k, 0]) * vx[i, j] + (u[i, j, k+1, 0] - u[i, j, k-1, 0]) * wx[i, j, k])
        vdy = 0.5 * ((v[i+1, j, k, 0] - v[i-1, j, k, 0]) * uy[i, j] + (v[i, j+1, k, 0] - v[i, j-1, k, 0]) * vy[i, j] + (v[i, j, k+1, 0] - v[i, j, k-1, 0]) * wy[i, j, k])
