SUBROUTINE mgrid(p,dtime,edt,cfcdiv)
   USE header, ONLY : NI,NJ,NK,maxout,maxint,int1,ngrid,rc_kind
   !     --------------
   ...
   REAL(kind=rc_kind), PARAMETER :: tol_r=1d-10
...
SUBROUTINE mgriddup(nx, ny, nz, rhs, cp, p,dtime,edt,cfcdiv)
   USE header, ONLY : NI,NJ,NK,maxout,maxint,int1,ngrid,rc_kind
   !     --------------
   !     multigrid solver for the elliptic equation Del2 p= f
   !     Run pgrogram "preprocess" to get the value of maxout
