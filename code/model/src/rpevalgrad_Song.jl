...
# c     solid boundaries
   ...
   for i in 1:NI
      ...
      for j in 1:NJ-1
         ...
         dzsig[j, 2] = 0.5e0 * (zc[i+1, j+1, k+3] + zc[i+1, j+2, k+3] - (zc[i+1, j+1, k+2] + zc[i+1, j+2, k+2]))
         for k in 1:NK-1
         ...
         dzsig[j, NK+1] = 0.5e0 * (zc[i+1, j+1, NK+1] + rho[i+1, j+2, NK+1] - (zc[i+1, j+1, NK] + zc[i+1, j+2, NK])) * DL
      end
...
# c     periodic boundaries
   ...
   for j in 1:NJ
      ...
      for i in 1:NI-1
         ...
         dzsig_x[i+1, 1] = 0.5e0 * (zc[i+1, j+1, k+3] + zc[i+2, j+1, k+3] - (zc[i+1, j+1, k+2] + zc[i+2, j+2, k+2])) * DL
         for k in 1:NK-1
      ...
      dzsig_x[i+1, 1] = 0.5e0 * (zc[i+1, j+1, k+3] + zc[2, j+1, k+3] - (zc[i+1, j+1, k+2] + zc[2, j+1, k+2])) * DL
      for k in 1:NK-1
      ...
      for i in 1:NI
         ...
         for k in NK-1:-1:1
            drpx[i,j,k] = drpx[i,j,k+1] + vconst * ux[i+1, j+1] * 0.5e0 * (Jx[i+1, NK+1] + Jx[i, NK+1])
         end
      end
   end
