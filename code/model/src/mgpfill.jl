      ...
      if (@fortGet("qpr", rc_kind) != 0e0)
            ...
            for i in 1:NI
                  for j in 1:NJ
                        ...
                        local psig = gin * (-skfc[i, j, k] - gqk[i, j, k, 1] * dpdxi - gqk[i, j, k, 2] * dpdelta + edt * (czf[i, j, k] - wfbcb[i, j]))
            ...
            for i in 1:NJ
                  ...
                  local psig = gin * (-skfc[i, j, k] - gqk[i, j, k, 1] * dpdxi - gqk[i, j, k, 2] * dpdelta)
                  pf[i+1, j+1, 1] = pf[i+1, j+1, 2] - psig
            end
            local j = NJ + 1
            for i in 1:NJ
            ...
            for i in 1:NI
                  for k in 1:NK
                        ...
                        local peta = gin * (-gqj[i, j, k, 1] * dpdxi - gqj3[i, j, k] * dpdsig + (edt * (cyf[i, j, k] - vfbcs[i, k]) - sjfc[i, j, k]))
            ...
            for i in 1:NI
                  for k in 1:NK
                        ...
                        local dpdxi = 0.25e0 * (pf[i+2, j+2, k+1] + pf[i+2, j+1, k+1] - pf[i, j+2, k+1] - pf[i, j+1, k+1])
                        local dpdsig = 0.25e0 * (pf[i+1, j+2, k+2] + pf[i+1, j+1, k+2] - pf[i+1, j+2, k] - pf[i+1, j+1, k])
                        local peta = gin * (-gqj[i, j, k, 1] * dpdxi - gqj3[i, j, k] * dpdsig + (edt * (cyf[i, j, k] - vfbcn[i, k]) - sjfc[i, j, k]))
