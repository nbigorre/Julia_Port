    ...
    for k in 1:NK
        for j in 1:NJ
            for i in 1:NI
                ...
                local Wdsig = 0.5e0 * ((u[i+1, j+1, k+2, n+1] * wx[i+1, j+1, k+2] + v[i+1, j+1, k+2, n+1] * wy[i+1, j+1, k+2] + EPS * w[i+1, j+1, k+2, n+1] * wz[i+1, j+1, k+1]) * Jac[i, j, k+1]
                                    - (u[i+1, j+1, k, n+1] * wx[i+1, j+1, k] + v[i+1, j+1, k, n+1] * wy[i+1, j+1, k] + EPS * w[i+1, j+1, k, n+1] * wz[i+1, j+1, k]) * Jac[i, j, k-1])
