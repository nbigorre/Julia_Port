    ...
    for selvar in 1:5+ntr
        ...
        if (av_comp[selvar] == 1)
            ...
            if (cppdefs.implicit)
                if (selvar < 4.5)
                    for i in 1:NI
                        for j in 1:NJ
                            ...
                            mat_D[i,j,1:NK, selvar] .= 0e0 .- var0[i+1, j+1, 2:NK+1] - (dtimel * JacInv[i, j, 1:NK] * mat_D[i,j,1:NK, selvar]) + (dtimel * JacInv[i, j, 1:NK] * uvarx_advec[i+1,j+1,2:NK+1]) 
                ...
                local lc_diff = @inbounds @views @. dtimel * JacInv[1:NI, 1:NJ, 1:NK] .* uvarx[2:NI+1, 2:NJ+1, 2:NK+1]
                if (selvar == 1)
                ...
                if (selvar == 3)
                    @inbounds @views @. cx[1:NI, 1:NJ, 1:NK] = u[2:NI+1, 2:NJ+1, 2:NK+1, 1] - lc_diff
                end
                if (selvar == 4)
                    @inbounds @views @. cy[1:NI, 1:NJ, 1:NK] = v[2:NI+1, 2:NJ+1, 2:NK+1, 1] - lc_diff
                end
            end
            if (selvar == 5)
                local lc_diff = @inbounds @views @. dtimel * JacInv[1:NI, 1:NJ, 1:NK] .* uvarx[2:NI+1, 2:NJ+1, 2:NK+1]
                
                @inbounds @views @. cz[1:NI, 1:NJ, 1:NK] = w[2:NI+1, 2:NJ+1, 2:NK+1, 1] - lc_diff
                for it in 1:ntr
                    if (selvar == 5 + it)
