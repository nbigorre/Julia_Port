function ini_setup(pcorr)
  ...
  for j in 0:div(NJ,2)
    local Fy = tanh((rc_kind(j) - rc_kind(150)) / rc_kind(100) * 2e0 * PI)
    dyM[j] = (Fy - 3e0) / (-4e0) * dy
  end
  for j in div(NJ,2)+1:NJ+1
    Fy = tanh((rc_kind(j) - rc_kind(NJ - 150)) / rc_kind(100) * 2e0 * PI)
    dyM[j] = (Fy + 3e0) / (4e0) * dy
  end
  yc[0] = -0.5e0 * dyM[0] * 1e-3
  for j in 1:NJ+1
    yc[j] = yc[j-1] + dyM[j-1] * 1e-3
  end
