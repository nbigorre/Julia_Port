...
function ini_st()
   ...
   local npl = 33
   local bs1 = zeros(rc_kind, npl)
   local cs1 = zeros(rc_kind, npl)
   local ds1 = zeros(rc_kind, npl)
   local bT1 = zeros(rc_kind, npl)
   local cT1 = zeros(rc_kind, npl)
   local dT1 = zeros(rc_kind, npl)
   local depoff = zeros(rc_kind, npl)
   local alpha = 1.6e-4
   local dep = [
      -5500e0, -5000e0, -4500e0, -4000e0, -3500e0,
   ...
   local svert = [
      34.76, 34.87, 34.91, 34.94, 35.00, 35.05, 35.08,
      35.11, 35.13, 35.13, 35.11, 35.05, 34.99, 34.94,
      34.89, 34.91, 34.89, 34.89, 34.88, 34.89, 34.90,
      34.88, 34.88, 34.88, 34.88, 34.88, 34.87, 34.81,
      34.76, 34.75, 34.75, 34.74, 34.71
   ...
   local tvert = [
      1.61, 1.72, 1.77, 1.78, 1.87, 2.18, 2.61, 3.05,
      3.28, 3.62, 3.80, 4.06, 4.32, 4.57, 4.84, 5.29, 5.71,
      6.37, 6.96, 7.83, 8.75, 9.82, 10.42, 11.00, 11.71, 12.12,
      12.63, 13.28, 14.46, 16.01, 16.76, 17.04, 17.25
   ]
   ...
   tightness = 0.03e0
   bfsqbkgrnd = 1e-6
   dtdz = bfsqbkgrnd / (gpr * 10e0) / alpha
   dsdz = 0e0

   for k in 1:npl
      depoff[k] = dep[k] - mldepth
   end
   spline(npl, depoff, svert, bs1, cs1, ds1)
   spline(npl, depoff, tvert, bT1, cT1, dT1)
   ...
   for j in 0:NJ+1
      for i in 0:NI+1
         for k in 0:NK+1
            z = DL * zc[i, j, k]
            if (z >= -mldepth)
               Tbkgrnd = seval(npl, -1e0 * mldepth, depoff, tvert, bT1, cT1, dT1) - (z + mldepth) * dtdz
               sbkgrnd = seval(npl, -1e0 * mldepth, depoff, svert, bs1, cs1, ds1) - (z + mldepth) * dsdz
   ...
   for j in 0:NJ+1
      ...
      for i in 0:NI+1
         yfront = 0.5e0 * (yc[NJ+1] + yc[0]) + amplitude * sin(2e0 * PI * xc[i] / (0.5 * (xc[NI+1] + xc[NI]) / wiggles))
         ...
         for k in 1:NK
            ...
            T[i, j, k, 0] = slfacnew * (thy - 1e0) + T[i, NJ, k, 0]
