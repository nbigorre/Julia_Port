...
function wind_stress(udif,vdif,step)
    ...
    for j in 1:div(NJ,2)
        stress_top_y[1,j] = stressmax * 0.5 * (1e0 + tanh(((yc[j+1] * 1e3 - yset) / yw) * PI)) 
    end
    local yset = yc[NJ+1] * 1e3 - y0 - yw / 2e0
    ...
    for j in 1:NJ
        for i in 1:NI
            ...
            local rhoinv = 1e0 / rho[i+1,j+1,NK+1]
            
            local Kdudzt = stress_top_x[i,j] * rhoinv * fact
            local Kdvdzt = stress_top_y[i,j] * rhoinv * fact
            
            udif[i,j,NK] = fac * Jac[i+1,j+1,NK+1] * wz[i+1,j+1,NK+1] * Kdudzt
            vdif[i,j,NK] = fac * Jac[i+1,j+1,NK+1] * wz[i+1,j+1,NK+1] * Kdvdzt
        end
    end
